...
MSG_GROUP_MESSAGE   = 4
MSG_SERVER_INFO     = 5
MSG_CREATE_GROUP    = 6   # create or join group

# Old simple signup (legacy only)
MSG_SIGNUP_REQUEST  = 7
MSG_SIGNUP_RESPONSE = 8

# New multi-step signup flow
MSG_SIGNUP_EMAIL_REQUEST     = 9    # step 1
MSG_SIGNUP_EMAIL_RESPONSE    = 10

MSG_VERIFY_EMAIL_REQUEST     = 11   # step 2
MSG_VERIFY_EMAIL_RESPONSE    = 12

MSG_SET_CREDENTIALS_REQUEST  = 13   # step 3
MSG_SET_CREDENTIALS_RESPONSE = 14
...
# Priorities
PRIORITY_CONTROL = 1   # login, signup, metadata
PRIORITY_CHAT    = 2   # normal chat messages
PRIORITY_FILE    = 3   # reserved for file transfer

# Packet structure: version (1B), type (1B), priority (1B), reserved (1B), length (4B)
HEADER_FORMAT = "!BBBBI"
HEADER_SIZE = struct.calcsize(HEADER_FORMAT)


def _recv_exact(sock, length: int) -> bytes | None:
    """Receive exactly 'length' bytes, return None if socket closed."""
    data = b""
...
def send_packet(sock, msg_type: int, priority: int, payload_dict: dict) -> None:
    """Encode JSON payload + header and send as one packet."""
...
def recv_packet(sock):
    """
    Receive one full packet.  
    Returns (msg_type, priority, payload_dict)  
    or (None, None, None) if connection closed/broken.
    """
